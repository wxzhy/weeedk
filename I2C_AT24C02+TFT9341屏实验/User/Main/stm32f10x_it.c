...
{
	if(EXTI_GetITStatus(EXTI_Line11)!= RESET)  	 //判断Line11线上的中断是否发生PD11 
	{  
		EXTI_ClearITPendingBit(EXTI_Line11);   
		Flag = 0x01;
	}

	if(EXTI_GetITStatus(EXTI_Line12)!= RESET)   //判断Line12线上的中断是否发生PD12
	{  
		EXTI_ClearITPendingBit(EXTI_Line12);   
		Flag = 0x02;
	}    
	if(EXTI_GetITStatus(EXTI_Line13)!= RESET)   //判断Line13线上的中断是否发生PC13
	{  
