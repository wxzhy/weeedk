...
///******************************************************************
//函数名：  Gui_Drawbmp16
//功能：    显示一副16位BMP图像 大小：Xm*Ym，生成字节数＝Xm*Ym*2
//输入参数：x,y :起点坐标
// 			*p :图像数组起始地址
//返回值：  无
//修改记录：无
//******************************************************************  
void Gui_Drawbmphua16(u16 x,u16 y,const unsigned char *p) //显示240*160 图片
{
  int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+240-1,y+152-1);//窗口设置
   for(i=0;i<240*160;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
...
void Gui_Drawbmp16(u16 x,u16 y,const unsigned char *p) //显示240*160 图片
{
  int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+240-1,y+152-1);//窗口设置
...
void Welcome(void)
{
			DrawTestPage("DAC应用-定时器触发DMA传输实验");
			Gui_StrCenter(0,25,BRED,YELLOW,"南航 NUAA_CM3_107 KIT",16,0);//居中显示
	if (USE_HORIZONTAL==1)	Show_Str(85,65,GREEN,BLACK,"嵌入式系统实验",24,1);
	else					Show_Str(30,45,GREEN,BLACK,"嵌入式系统实验",24,1);
	Gui_StrCenter(0,76,RED,WHITE,"DAC输出正弦波演奏东方红乐曲",16,0);//居中显示	
	Gui_StrCenter(0,100,WHITE,BLACK,"KEY1键奏乐 KEY2键停止!",16,1);//居中显示	
