...
void Welcome(void)
{
	DrawTestPage("SysTick定时器实验测试程序");
	
	Gui_StrCenter(0,40,BLACK,WHITE,"南航 NUAA_CM3_107 KIT",16,0);//居中显示
...
//Gui_StrCenter(0,46,RED,BLUE,"嵌入式系统原理及应用配套实验板",16,1);//居中显示
	Gui_StrCenter(0,100,WHITE,BLACK,"SysTick定时器实验测试程序",16,0);//居中显示	
	Gui_StrCenter(0,130,WHITE,BLACK,"按KEY1定时 100ms",16,0);//居中显示	
	Gui_StrCenter(0,150,WHITE,BLACK,"按KEY2定时 250ms",16,0);//居中显示	
	Gui_StrCenter(0,170,WHITE,BLACK,"按KEY3定时 500ms",16,0);//居中显示	
	Gui_StrCenter(0,190,WHITE,BLACK,"按KEY4定时1000ms",16,0);//居中显示	
	Gui_StrCenter(0,220,YELLOW,BLACK,"SysTick定时器定时时间(ms):",16,0);//居中显示
	
			POINT_COLOR = WHITE;
			BACK_COLOR = BLACK;  
			
			LCD_ShowNum(100,250,50,4,16);			
