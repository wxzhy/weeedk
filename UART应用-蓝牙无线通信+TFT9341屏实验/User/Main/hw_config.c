...
*出口参数：无
*
*功能说明：NVIC初始化配置， 使能UART2串口接收中断
**********************************************/
void NVIC_Configuration(void)
{
	NVIC_InitTypeDef NVIC_InitStructure;
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);			  //中断分组2
	NVIC_InitStructure.NVIC_IRQChannel = USART3_IRQn; 		  //USART2接收中断
...
void GPIO_Configuration(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = LED1_PIN | LED2_PIN | LED3_PIN | LED4_PIN|IA_PIN|IB_PIN ;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;	
	GPIO_Init(GPIOD, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = LED5_PIN|IA_PIN|IB_PIN|GPIO_Pin_1 | GPIO_Pin_2;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
...
/*********************************************
*函数名称：void USART_BHT_Configuration(void)
*
*入口参数：无
*
*出口参数：无
*
*功能说明：USART3 (蓝牙模块)初始化配置 包括GPIO初始化 TX必须配置为复用输出
**********************************************/
