...
{
value_buf[count]=Read_ADC1_MultiChannel(ADC_Channel,Rank);   //获取采样值
//delay();
}
...
for(count=0;count<N;count++)
{
sum+=Read_ADC1_MultiChannel(ADC_Channel,Rank);
//delay();
}
return (u16)(sum/N);
}
...
u8 count;
int sum=0;
value_buf[i++]=Read_ADC1_MultiChannel(ADC_Channel,Rank);
if(i==N) i=0;
for (count=0;count<N;count++)
sum+=value_buf[count];
return (u16)(sum/N);
...
new_value=Read_ADC1_MultiChannel(ADC_Channel,Rank);
