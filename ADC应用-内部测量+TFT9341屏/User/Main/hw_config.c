...
void ADC_Configuration(void)
{
	...
	ADC_InitStructure.ADC_ContinuousConvMode = DISABLE;		  	/*禁止连续模数转换模式*/
...
*
***************************************************/
	int Read_ADC1_MultiChannel(u8 channNo,u8 Rank)
{  
u16  ADC_data;
    ADC_RegularChannelConfig(ADC1, channNo, Rank, ADC_SampleTime_239Cycles5 );/* 设置指定channNo通道及采样率*/
		ADC_SoftwareStartConvCmd(ADC1, ENABLE);		 		/*使能ADC1,软件启动AD变换 */
...
void GPIO_Configuration(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = LED5_PIN|GPIO_Pin_1|GPIO_Pin_2;//PB1和PB2控制双色发光管，1亮，0灭
	GPIO_Init(GPIOB, &GPIO_InitStructure);
