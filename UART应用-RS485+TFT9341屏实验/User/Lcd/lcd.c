...
{
	if (MasterSlave==0) 	
			{DrawTestPage("UART-RS485从机模式实验");
			Gui_StrCenter(0,100,RED,YELLOW,"UART-RS485从机模式实验",16,0);//居中显示	
			}
	else 		
			{DrawTestPage("UART-RS485主机模式实验");
			Gui_StrCenter(0,100,RED,YELLOW,"UART-RS485主机模式实验",16,0);//居中显示	
			}
	
	Gui_StrCenter(0,40,BRED,YELLOW,"南航 NUAA_CM3_107 KIT",16,1);//居中显示
...
//Gui_StrCenter(0,46,RED,BLUE,"嵌入式系统原理及应用配套实验板",16,1);//居中显示
	Gui_StrCenter(0,130,WHITE,BLACK,"连RS-485到PC机",16,0);//居中显示
	Gui_StrCenter(0,150,YELLOW,BLACK,"打开PC上的串口助手",16,0);//居中显示
	Gui_StrCenter(0,170,YELLOW,BLACK,"按照通信协议发01命令",16,0);//居中显示
	Gui_StrCenter(0,190,YELLOW,BLACK,"查看PC串口助手接收到的信息",16,0);//居中显示
	Gui_StrCenter(0,210,YELLOW,BLACK,"通过RS-485接收的命令:",16,0);//居中显示
