...
//修改记录：无
//******************************************************************  
void Gui_Drawbmp16(u16 x,u16 y,const unsigned char *p) //显示240*96 图片
{
  	int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+240-1,y+96-1);//窗口设置
    for(i=0;i<240*96;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
...
{	
	...
	Show_Str(10,100,WHITE,BLACK,"有操作系统下的综合实验:",16,0);//居中显示
	Show_Str(10,120,WHITE,BLACK,"1.请填写自己的学号和姓名",16,0);//居中显示
	Show_Str(10,140,WHITE,BLACK,"2.任务调度及消息邮箱操作",16,0);//居中显示
	Show_Str(10,160,WHITE,BLACK,"3.ADC应用:PC2作为ADC1_IN12",16,0);//居中显示
	Show_Str(10,180,WHITE,BLACK,"4.DAC应用:PA4作为DAC输出",16,0);//居中显示
	Show_Str(10,200,WHITE,BLACK,"5.USART应用:串行通信",16,0);//居中显示
	
	Show_Str(20,240,WHITE,BLACK,"电位器电压值",16,0);	

	Show_Str(200,240,WHITE,BLACK,"mV",16,0);	

POINT_COLOR =YELLOW;BACK_COLOR =RED; 
	LCD_DrawRectangle(0,230,240,265);
	POINT_COLOR =RED;
	LCD_DrawRectangle(1,231,238,264);
	POINT_COLOR =YELLOW;
	LCD_DrawRectangle(2,232,237,263);
...
{
			DrawTestPage("嵌入式系统原理及应用综合实验");
			Gui_StrCenter(0,100,RED,YELLOW,"uC/OS-II下的消息邮箱实验",16,0);//居中显示	
	
	Gui_StrCenter(0,130,RED,YELLOW,"   uC/OS-II初始化过程   ",16,0);//居中显示

	Gui_StrCenter(0,150,YELLOW,BLACK,"建立消息邮箱",16,0);//居中显示
	Gui_StrCenter(0,170,YELLOW,BLACK,"创建多个任务",16,0);//居中显示
	Gui_StrCenter(0,190,YELLOW,BLACK,"启动任务调度",16,0);//居中显示
	Gui_StrCenter(0,210,WHITE,BLACK,"查看LED1并听蜂鸣器声响情况 ",16,0);//居中显示					
	Gui_StrCenter(0,230,WHITE,RED,"查看串口助手接收的提示信息",16,0);//居中显示					
//		Gui_Drawbmp16(0,30,gImage_uCOSII);
}
