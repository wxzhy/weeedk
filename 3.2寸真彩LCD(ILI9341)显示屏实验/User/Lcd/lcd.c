...
void Gui_Drawbmp16(u16 x,u16 y,const unsigned char *p) //显示xL*yL 图片
{
  	int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+xL-1,y+yL-1);//窗口设置
    for(i=0;i<xL*yL;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
...
//Gui_StrCenter(0,46,RED,BLUE,"嵌入式系统原理及应用配套实验板",16,1);//居中显示
	Gui_StrCenter(0,100,RED,BLUE,"TFT测试程序",16,1);//居中显示	
	Gui_StrCenter(0,120,YELLOW,BLUE,"3.2' ILI9341 240X320",16,1);//居中显示
}
