...
#define	LCD_LED PBout(14) //LCD背光    		 PB14=1打开背光，0关闭背光 
//如果使用官方库函数定义下列底层，速度将会下降到14帧每秒，建议采用我司推荐方法
//以下IO定义直接操作寄存器，快速IO操作，刷屏速率可以达到28帧每秒！ 

#define	LCD_CS_SET  GPIO_SetBits(GPIOC,GPIO_Pin_6) /*片选端口	PC6*/
#define	LCD_RS_SET	GPIO_SetBits(GPIOD,GPIO_Pin_13) /*数据/命令PD13*/   
#define	LCD_WR_SET	GPIO_SetBits(GPIOD,GPIO_Pin_14) /*写数据	PD14*/
#define	LCD_RD_SET	GPIO_SetBits(GPIOD,GPIO_Pin_15) /*读数据	PD15*/
#define	LCD_CS_CLR  GPIO_ResetBits(GPIOC,GPIO_Pin_6) /*片选端口PC6*/
#define	LCD_RS_CLR	GPIO_ResetBits(GPIOD,GPIO_Pin_13) /*数据/命令PD13*/   
#define	LCD_WR_CLR	GPIO_ResetBits(GPIOD,GPIO_Pin_14) /*写数据	PD14*/
#define	LCD_RD_CLR	GPIO_ResetBits(GPIOD,GPIO_Pin_15) /*读数据	PD15*/
#define 	DATAOUT(x) 	GPIO_Write(GPIOE,x);			 /*数据输出*/
