...
void Welcome(void)
{
	DrawTestPage("红外控制测试程序");
	
	Gui_StrCenter(0,40,BRED,YELLOW,"南航 NUAA_CM3_107 KIT",16,0);//居中显示
	//Gui_StrCenter(0,60,RED,BLUE,"嵌入式系统配套实验开发板",16,1);//居中显示
	if (USE_HORIZONTAL==1)	Show_Str(45,65,GREEN,BLACK,"嵌入式系统实验开发板",24,0);
	else					Show_Str(10,65,GREEN,BLACK,"嵌入式系统实验开发板",24,0);
//Gui_StrCenter(0,46,RED,BLUE,"嵌入式系统原理及应用配套实验板",16,1);//居中显示
	Gui_StrCenter(0,100,RED,BLACK,"红外控制测试程序",16,0);//居中显示	
	Gui_StrCenter(0,130,YELLOW,BLACK,"通过红外接收的命令",16,0);//居中显示	
	LCD_ShowString(86, 160, 16,"        ",0);
	Gui_StrCenter(0,190,WHITE,BLACK," 电源键=电机停止 ",16,0);//居中显示
	Gui_StrCenter(0,190,WHITE,BLACK," 无红外遥控命令  ",16,0);//居中显示
}
