...
void Welcome(void)
{
			DrawTestPage("RTC应用日历实验测试程序");
			Gui_StrCenter(0,30,BRED,YELLOW,"  南航 NUAA_CM3_107 KIT  ",16,0);//居中显示
	if (USE_HORIZONTAL==1)	Show_Str(45,55,YELLOW,BLACK,"嵌入式系统实验开发板",24,0);
	else					Show_Str(10,55,GREEN,BLACK,"嵌入式系统实验开发板",24,0);
//Gui_StrCenter(0,46,RED,BLUE,"嵌入式系统原理及应用配套实验板",16,1);//居中显示
	Gui_StrCenter(0,90,WHITE,BLACK,"RTC日历实验测试程序",16,0);//居中显示	

	POINT_COLOR =YELLOW;BACK_COLOR =RED; 
	LCD_DrawRectangle(0,140,240,260);
	POINT_COLOR =RED;
	LCD_DrawRectangle(1,141,238,259);
	POINT_COLOR =YELLOW;
	LCD_DrawRectangle(2,142,237,258);
	LCD_DrawRectangle(3,143,236,257);
	POINT_COLOR =RED;
}
